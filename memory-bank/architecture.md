...
Subpackages:
- `harness/` — `core.py` (`Harness`), `interpreter.py` (`run_task_graph` — the fixed execute→verify→repair→halt walker), `touchpoints.py` (`Touchpoint` registry — every place the harness invokes a model), `dispatch.py` (`build_agent`, `run_subagent`, `make_delegate_tool`, `DispatchContext`, `ERROR_PREFIX`, `MAX_ITERATIONS` — single agent-construction + nested-dispatch point used by all harness paths), `bridge.py` (bus-to-queue bridge — single translation from `agent/llm` event vocabulary into `AgentEvent` taxonomy, shared by the solo and graph paths), `scaling.py`, `tool_scope.py`, `turn.py`
- `pipeline/` — `estimate.py` (`ScopeEstimate`, `Estimator` — three-rung scope classifier), `plan.py` (`Task`, `TaskGraph`, `parse_task_graph`, `ROOT_AGENT`), `sequencer.py` (`Sequencer` — builds the `TaskGraph`), `verifier.py` (`Verdict` — cold SUPP-tier LLM call that checks a coding step's diff against its instruction; fail-open on parse/API error)
...
- `tools/` — `__init__.py` (`make_tools`, `ExternalGrantCallback`, `HiddenGrantCallback`), `catalog.py` (tool-name groups: `READ_TOOLS`/`EDIT_TOOLS`/`FS_TOOLS`/`SHELL_TOOLS`/`ALL_TOOLS`
  — single source of truth for subagent allowlists and `<tools>` prompt generation), `files.py` (`ExternalGrantCallback`, `HiddenGrantCallback`, file tools), `shell.py`
...
Root has no `delegate` tool and cannot self-spawn. Sequencer-owned cross-agent dispatch (step 3 below) is controlled by the interpreter, not by an LLM deciding mid-turn to call a tool. Subagents that declare `delegates_to` receive a `delegate` tool built by `make_delegate_tool` (`agent/harness/dispatch.py`) — depth-capped at 1 (`can_delegate=False` on the child), capability-tightening only (child tool set intersected against the parent's effective set). Currently `code-expert`, `code-fixer`, `code-refactorer`, `complexity-remover`, `omni-worker`, `test-expert`, `test-fixer` declare `delegates_to`; root is excluded by construction (`subagent is None` branch in `build_agent`). Cross-agent dispatch flow:
...
3. `agent/harness/interpreter.py::run_task_graph()` walks the graph: `execute → verify → repair →
   ...
   `agent/harness/dispatch.py::run_subagent(agent, task, ctx=..., can_delegate=True)`
4. `run_subagent()` resolves `agent` against `SUBAGENTS` (any roster name, invocable or
   post-planning-only), builds a cold nested `Agent` via `build_agent(..., subagent=resolved, can_delegate=True)`
   (`prior=[]`; root never in this path; if the resolved subagent has `delegates_to`, it receives
   `make_delegate_tool` depth-capped at 1), runs it, and returns the last
...
Every touchpoint (`estimator`/`sequencer`/`root-dispatch`/`subagent-dispatch`/`verifier`/`micro` —
...
`<tools>` block is appended afterward by `dispatch.build_agent`, not by the subagent — see below

`dispatch.build_agent(resolved, working_dir, permissions, permission_callback, system_base, bus=None, subagent=None, hidden_grant_callback=None, external_grant_callback=None, tools_override=None, can_delegate=True)` (`agent/harness/dispatch.py`) is the **single point** that computes
the effective tool set and assembles the final system string, for all dispatch modes:
1. `effective` permissions = `permissions` ANDed field-wise with `subagent.permissions` (when set)
2. `selected` = `make_tools(working_dir, grant_cb=hidden_grant_callback, external_grant_cb=external_grant_callback)` filtered by `subagent.tools` allowlist (when set), then by whether `effective` grants each tool's `required_permission` (when there's no `permission_callback` to escalate)
3. if `tools_override` set: further filter `selected` to that set
4. if `subagent.delegates_to` non-empty and `can_delegate`: append `make_delegate_tool(subagent.delegates_to, ctx, parent_tools)` — root excluded by construction (`subagent is None`)
5. `system = system_base + "\n<tools>\n" + render_tool_instruction([t.name for t in selected])`
6. construct `Agent(system=system, …)`, register exactly `selected`, attach `PermissionGate(effective, …)`
...
`Harness.stream(session, user_input, permission_callback=None, hidden_grant_callback=None, external_grant_callback=None, seed=None)`
selects `system_base` by `seed` — `ROOT_SYSTEM_PROMPT` (root, no `seed`) vs `resolved_subagent.build_system_base()` (seed-dispatched subagent) — then calls `dispatch.build_agent`. `extra_params`
resolution in the no-graph path (see `## Estimator`): the Estimator's `chat` rung yields
`resolve_thinking_params(root_dispatch_resolved.model, enabled=False)` (root's model's explicit
thinking-disable payload, plan 34 — not a bare `{}`) and every other rung yields
`root_dispatch_resolved.extra_params` (the root-dispatch tier's resolved params). Direct mode passes
...
after `build_agent` returns (`agent.system` is the true assembled prompt, a mutable field on
