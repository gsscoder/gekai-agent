...
3. `_init_session()`: mounts BANNER widget; starts with an empty `workspace` dict (no cache file is read or written); starts session via `agent.start_session()`; loads `scope_gate` from settings; sets context limit from settings or model-prefix lookup; updates `#context-bar`; replays restored messages as USER/ASSISTANT widgets; if `needs_permissions` → calls `_ask_choice()` with `PERMISSION_CHOICES`, resolves + saves result
