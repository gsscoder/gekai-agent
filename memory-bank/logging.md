...
Multi-step plans no longer exist (dissolved into the sequencer + interpreter — see `architecture.md → Cross-Agent Dispatch`); there is no `step` field any more. Graph-spawned subagent steps are `run_subagent` calls from the interpreter, tracked inside the `harness` event. Root never holds a `delegate` tool; subagents that declare `delegates_to` can use it to call a narrower specialist, and those nested calls surface as `delegate` counts inside the `harness` event's `tools` dict

## Event Catalog
Process-level (once per run):
| Event | Where | Fields |
|---|---|---|
| `run.start` | `GekaiAgent.__init__` | `version`, `platform`, `estimator_model`, `sequencer_model`, `root_dispatch_model`, `subagent_dispatch_model`, `verifier_model`, `tiers_configured`, `permissions={read,write,exec}`, `verbose_telemetry` |
...
| `delegation` | `host`, `delegate`, `namespace`, `status` (`ok`/`failed`), `files`, `files_touched`, `summary_len`, `budget_exhausted` | only when a seed-dispatched subagent (`seed` forced `/`-slash route); mirrors `SubagentResult`. Nested calls a subagent makes via its own `delegate` tool are not separately logged here — they surface inside the `harness` event's `tools` count as `delegate` calls |
