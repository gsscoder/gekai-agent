...
_SEQUENCER_PROMPT = (
    ...
    "unneeded one is pure waste (an extra cold-context run, extra latency, extra tokens). never "
    "place a discovery step as the only step or the last step — it exists to feed a later step, "
    "never to end the graph. never place one when the request already names the specific "
    "file(s), symbol(s), or directory to touch. "
    ...
    "discovery step is only worth inserting when a later step's instruction actually references "
    "it this way — otherwise its findings are dropped on the floor. "
