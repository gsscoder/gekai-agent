...
from agent.tiers.catalog import DEFAULT_MODEL_CATALOG, ModelCatalogEntry, TierBinding, TierName
...
def test_load_model_catalog_is_a_live_read_of_the_code_side_catalog() -> None:
    ...
    loaded = tiers_store.load_model_catalog()
...
def test_load_model_catalog_reflects_catalog_additions_live(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(tiers_store, "DEFAULT_MODEL_CATALOG", (*DEFAULT_MODEL_CATALOG, extra))
...
def test_tier_binding_round_trip() -> None:
    ...
    tiers_store.save_tier_binding(TierName.SUPP, binding)

    loaded = tiers_store.load_tier_bindings()
    assert loaded[TierName.SUPP] == binding


def test_tier_binding_save_overwrites_same_tier() -> None:
    tiers_store.save_tier_binding(TierName.CORE, TierBinding(model="a", default_effort="low"))
    tiers_store.save_tier_binding(TierName.CORE, TierBinding(model="b", default_effort="high"))
...
def _seed_catalog(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(tiers_store, "DEFAULT_MODEL_CATALOG", tuple(TIER_CATALOG.values()))
...
def _seed_catalog_and_bindings(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    for tier, binding in TIER_BINDINGS.items():
        tiers_store.save_tier_binding(tier, binding)
...
def test_tiers_configured_false_when_only_partially_bound(monkeypatch: pytest.MonkeyPatch) -> None:
    _seed_catalog(monkeypatch)
    tiers_store.save_tier_binding(TierName.FAST, TIER_BINDINGS[TierName.FAST])
...
def test_tiers_configured_false_when_bound_but_missing_credential(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("agent.tiers.resolve.credentials.has_api_key", lambda name: name != "openai:fast-model")
